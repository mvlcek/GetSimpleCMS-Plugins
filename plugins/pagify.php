...
function pagify($content) {
  global $pagify_size, $pagify_pages, $metak;
...
#  $pageSize: size and optional unit, e.g. '1000' or '1000b' (100 bytes), '20w' (20 words), 100c (100 characters), 3p (3 paragraphs)
...
function return_pagify_content($content, $pageSize, $pageNum=0, $link=null) {
  if (!preg_match('/^(\d+)\s*([a-zA-Z]*)$/',$pageSize,$match)) return $content;
  $size = (int) $match[1];
...
#  $pageNum: page number to display
#  $link: link to go to other pages, must include a placeholder '%PAGE%' for the page number (starting with 0)
function return_pagify_navigation($numPages, $pageNum=0, $link=null) {
  global $pagify_pages;
  ...
  if ($numPages > 1) {
    ...
    if ($link == null) {
      ...
      if (defined('PAGIFY_SEPARATOR')) {
        ...
        $link .= strpos($link,'?') === false ? '?page=%PAGE%' : '&page=%PAGE%';
      }
    }
    $link = htmlspecialchars($link);
    $pagingHtml .= '<div class="paging">';
    if ($pageNum > 0) {
      $pagingHtml .= ' <span class="first active"><a href="'.str_replace('%PAGE%',1,$link).'" title="'.i18n_r('pagify/TITLE_FIRST').'">'.i18n_r('pagify/FIRST').'</a></span>';
      $pagingHtml .= ' <span class="previous active"><a href="'.str_replace('%PAGE%',$pageNum,$link).'" title="'.i18n_r('pagify/TITLE_PREVIOUS').'">'.i18n_r('pagify/PREVIOUS').'</a></span>';
    } else {
    ...
    for ($i=$min; $i<=$max; $i++) {
      if ($i == $pageNum) {
        ...
        $pagingHtml .= ' <span><a href="'.str_replace('%PAGE%',$i+1,$link).'" title="'.(trim($t) ? $t.($i+1) : '').'">'.($i+1).'</a></span>';
      }
    }
